...
namespace BTL_GK.Views
{
    ...
    {
        //2 đối tượng kết nối
        string strCon = @"Data Source=DESKTOP-44Q601U\SQLEXPRESS;Initial Catalog=Giay3;Integrated Security=True;Encrypt=False";
        ...
        private void lsvDSNV_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //dtpNgaySinh.Value = new DateTime(int.Parse(ngaySinh[2]), int.Parse(ngaySinh[0]), int.Parse(ngaySinh[1]));

            dtpNgaySinh.Value = new DateTime(int.Parse(ns[2]), int.Parse(ns[1]), int.Parse(ns[0]));
